...
- Uses GitLab CI with semantic release
- Pre-commit hooks run in CI with `uvx pre-commit run -a --show-diff-on-failure`
- Publishes wheels to NGC registry
- `UV_LINK_MODE: "copy"` configured to avoid hardlink issues
