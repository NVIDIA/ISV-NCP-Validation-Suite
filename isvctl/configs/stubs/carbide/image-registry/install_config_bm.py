...
"""Provision a bare-metal instance using OS config, proving OS+config works.

Uses ``carbidecli instance create`` with the OS config to provision
a BM instance. Maps to the image-registry template's ``install_config_bm``
step.

Usage:
    python install_config_bm.py --os-id <os-id> --site-id <site-id>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Provision BM instance with OS config")
    ...
    parser.add_argument("--name", default="ncp-bm-cfg-test")
    ...
    try:
        ...
        # Track for teardown
        state = load_state()
        state["bm_cfg_instance_id"] = instance_id
