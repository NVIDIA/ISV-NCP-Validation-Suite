...
"""Provision a bare-metal instance with a specific OS image.

Uses ``carbidecli instance create`` with the OS ID to provision
a BM instance, proving the OS image works on bare metal.
Maps to the image-registry template's ``install_image_bm`` step.

Usage:
    python install_image_bm.py --os-id <os-id> --site-id <site-id>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Provision BM instance with OS image")
    ...
    parser.add_argument("--name", default="ncp-bm-img-test")
    ...
    try:
        ...
        # Track for teardown
        state = load_state()
        state["bm_img_instance_id"] = instance_id
