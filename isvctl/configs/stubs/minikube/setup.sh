...
# Minikube configures kubectl automatically, so just use kubectl
...
if ! $KUBECTL cluster-info &> /dev/null 2>&1; then
    echo "Error: Cannot connect to Minikube cluster. Is it running? (minikube start)" >&2
    exit 1
fi
...
# Get node count (usually 1 for local minikube)
