...
| `query_node_health_agents` | test | `providers/shared/breakfix/query_node_health_agents.py` | `agents_observable`, `agents[].{node_id,agent_name,running}` -- SSH `systemctl` probe. Any name is accepted, but a `running` record must supply one; the check matches no specific product. List every GPU node, marking uncovered ones `running: false` rather than omitting them; skip when no GPU nodes are configured (BFX04-01) |
