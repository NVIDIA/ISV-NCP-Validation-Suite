...
| `query_node_health_agents` | test | `providers/my-isv/scripts/breakfix/query_node_health_agents.py` | `agents_observable`, `agents[].{node_id,agent_name,running}` -- any name is accepted, but a `running` record must supply one; the check matches no specific product. List every GPU node, marking uncovered ones `running: false` rather than omitting them; skip when the site has no GPU nodes (BFX04-01) |
