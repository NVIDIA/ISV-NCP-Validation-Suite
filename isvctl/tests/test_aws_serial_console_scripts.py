...
def _load_aws_common_script(script_name: str) -> ModuleType:
    ...
    script_path = AWS_COMMON_SCRIPTS / script_name
...
def test_aws_serial_console_helper_emits_retention_fields() -> None:
    ...
    assert result["retention_days_required"] == 31
    ...
    assert result["retention_limitations"]
...
def test_my_isv_bare_metal_demo_emits_passing_retention_evidence() -> None:
    ...
    assert result["retention_days_required"] == 31
    assert result["retention_days_configured"] == 31
    assert result["oldest_queryable_log_age_days"] == 31
