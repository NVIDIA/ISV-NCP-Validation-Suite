...
class TestGetEnvironmentConfig:
    ...
    def test_returns_custom_endpoint_when_set(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {"ISV_LAB_SERVICE_ENDPOINT": "https://custom.example.com"},
        ):
    ...
    def test_returns_defaults_when_env_not_set(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            # Should return some non-empty values from isvreporter defaults
            assert endpoint is not None
            assert len(endpoint) > 0
            assert ssa_issuer is not None
            assert len(ssa_issuer) > 0
