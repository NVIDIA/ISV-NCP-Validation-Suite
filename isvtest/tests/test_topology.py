...
class TestFailureDomainObservabilityCheck:
    """Tests for BmFailureDomainObservabilityCheck (STG05-01)."""

    def test_all_hosts_mapped_passes(self) -> None:
        """Every host reporting a failure domain passes."""
        check = BmFailureDomainObservabilityCheck(config={"step_output": _topology_output()})
        check.run()
    ...
    def test_single_domain_still_observable(self) -> None:
        ...
        check = BmFailureDomainObservabilityCheck(config={"step_output": _topology_output(hosts=hosts)})
        check.run()
        assert check._passed is True
    ...
    def test_unmapped_allowed_when_not_required(self) -> None:
        ...
        hosts = [_host("m-1", "rack-A"), _host("m-2", "")]
        check = BmFailureDomainObservabilityCheck(
    ...
    def test_min_failure_domains_met(self) -> None:
        ...
        check = BmFailureDomainObservabilityCheck(config={"step_output": _topology_output(), "min_failure_domains": 2})
        check.run()
        assert check._passed is True

    def test_step_failure_fails(self) -> None:
        """A failed step output fails the check."""
        check = BmFailureDomainObservabilityCheck(config={"step_output": _topology_output(success=False, error="boom")})
        check.run()
        assert check._passed is False
    ...
    def test_missing_hosts_list_fails(self) -> None:
        ...
        check = BmFailureDomainObservabilityCheck(config={"step_output": {"success": True}})
        check.run()
        assert check._passed is False

    def test_min_hosts_enforced(self) -> None:
        """Fewer hosts than min_hosts fails."""
        check = BmFailureDomainObservabilityCheck(config={"step_output": _topology_output(hosts=[]), "min_hosts": 1})
        check.run()
        assert check._passed is False
